    ...
    void Datmo::store_background()
    {
        ...
        for (int loop_hit = 0; loop_hit < nb_beams_; loop_hit++)
            background_[loop_hit] = r_[loop_hit];
    ...
    void Datmo::reset_motion()
    {
        ...
        for (int loop_hit = 0; loop_hit < nb_beams_; loop_hit++)
            dynamic_[loop_hit] = false;

    } // reset_motion

    void Datmo::detect_current_motion()
{
    // compare each beam with the stored background
    for (int i = 0; i < nb_beams_; i++)
    {
        if (std::fabs(r_[i] - background_[i]) > detection_threshold)
            dynamic_[i] = true;
        else
            dynamic_[i] = false;
    }
    ...
    void Datmo::detect_motion()
    {
        ...
        if (!current_robot_moving_)
        {
            ...
            if (!previous_robot_moving_)
            {
                ...
                if(stored_background_==false)
                {
                    store_background();
                    stored_background_=true;
                }
            }
            else
            {
            ...
            reset_motion();  // all dynamic[] = false
